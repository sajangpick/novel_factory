...
Novel Alchemist 대시보드에 접속하면 **좌측 사이드바**에 메뉴가 있습니다.

---

### 매일 쓰는 메뉴 (집필 흐름)

좌측 사이드바 상단에 4개 메뉴가 순서대로 있습니다. **위에서 아래로** 사용하면 됩니다.

---

#### ① 현재 상태

**화면 구성**: 상단에 "에피소드 동기화" 패널, 하단에 기억 카드 목록

**에피소드 동기화 패널**
- 모든 에피소드가 카드로 표시됩니다
- 각 카드에 상태가 색깔로 표시됩니다:
  - 🟢 **확정됨**: 기억 카드가 생성된 에피소드
  - 🟡 **미확정**: 본문은 있지만 아직 확정하지 않은 에피소드
  - 🟠 **수정됨**: 확정 후 본문이 변경된 에피소드 (재확정 필요)
  - 🔴 **삭제됨**: 기억 카드는 있지만 본문이 없는 에피소드
- **"확정" 버튼**: 클릭하면 AI가 해당 화를 분석하여 기억 카드 + 대시보드를 업데이트합니다
- **"롤백" 버튼**: 잘못 확정한 경우 되돌립니다

**기억 카드 목록**
- 확정된 에피소드마다 AI가 만든 요약 카드가 표시됩니다
- 이 카드들이 다음 화 집필 시 AI의 "기억"이 됩니다

---

#### ② 전략 브리핑

**화면 구성**: 상단에 현재 상황 요약, 중앙에 AI 제안 영역, 하단에 떡밥/캐스팅

**사용 순서**:
1. 화수를 확인합니다 (자동으로 다음 화가 선택됩니다)
2. **"AI 방향 제안"** 버튼을 클릭합니다
3. AI가 3가지 스토리 방향을 제안합니다 — 마음에 드는 것을 참고합니다
4. 필요하면 떡밥 관리, 캐릭터 캐스팅, 주의사항을 설정합니다

**참고**: AI 제안을 다시 받으면 비용이 발생합니다. 확인 팝업이 뜹니다.

---

#### ③ 본문 집필

**화면 구성**: 좌측에 텍스트 편집기, 우측에 버튼들

**새 에피소드 쓰기**:
1. 화수를 선택합니다
2. **"AI 본문 생성"** 버튼을 클릭합니다
3. AI가 이전 화 + 설계도 + 기억 카드를 참고하여 본문을 생성합니다
4. 생성된 본문을 읽고, 필요하면 직접 수정합니다
5. **"저장"** 버튼 → 파일과 데이터베이스에 동시 저장됩니다

**버튼 설명**:
| 버튼 | 기능 |
|------|------|
| AI 본문 생성 | AI가 새 본문을 만듭니다 |
| 저장 | 현재 내용을 파일 + DB에 저장합니다 |
| 확정하기 | 검수 없이 바로 확정합니다 (기억 카드 자동 생성) |
| 폐기 | 이 화의 본문을 삭제합니다 |
...
**화면 구성**: 좌측에 행번호가 붙은 본문, 우측에 "AI 검수 실행" 버튼, 하단에 빨간펜 지시창

이 페이지에서 **자동 검수 + 수동 교정** 두 가지를 모두 할 수 있습니다.

**[자동 검수] — "AI 검수 실행" 버튼**

1. 우측의 **"AI 검수 실행"** 버튼을 클릭합니다 (2~3분 소요)
2. AI가 참고자료(바이블, 인명록, 세계관 DB)와 본문을 대조하여 오류를 찾습니다
3. 발견된 이슈가 빨간펜 영역에 카드로 나타납니다:
   - **빨간색 "오류"**: 반드시 수정해야 하는 문제 (설정 충돌, 말투 위반, 현대어 등)
   - **노란색 "주의"**: 확인이 필요한 문제 (바이블에 없는 설정 등)
4. 각 이슈 카드에서:
   - **행번호 버튼** → 클릭하면 해당 행으로 스크롤됩니다
   - **"AI 수정" 버튼** → AI가 수정안을 만듭니다
   - 수정안이 나오면 **원본(빨간 취소선)**과 **수정본(초록색)**이 비교 표시됩니다
   - **"적용" 버튼** → 본문에 반영 + 자동 저장됩니다

**[수동 교정] — 빨간펜 지시창**

AI가 못 찾은 문제는 직접 지시합니다:

1. 본문에서 **문제가 있는 행을 클릭**합니다 → 입력창에 행번호가 자동 입력됩니다
2. 뒤에 지시 내용을 적습니다. 예시:
   - `194행: 현대어를 무협 용어로 고쳐줘`
   - `384행: 천마 편지 내용이 바이블에 없어`
   - `이 화에서 이준혁이 반말하는 곳 찾아줘`
3. **"지시" 버튼** 클릭 → AI가 분석 후 이슈 카드로 응답합니다
4. 이슈 카드에서 **"AI 수정" → "적용"** 으로 수정합니다

**[확정하기]**

검수와 수정이 끝나면 하단의 **"확정하기"** 버튼을 클릭합니다.
AI가 본문을 분석하여 기억 카드 + 대시보드를 자동 업데이트합니다.

---

### 관리 도구 (가끔 사용)

사이드바 하단의 **"관리 도구 ▼"** 를 클릭하면 펼쳐집니다.

| 메뉴 | 용도 | 언제 사용하나 |
|------|------|-------------|
| 인명록 관리 | 캐릭터 정보 검색/수정 | 새 캐릭터 추가하거나 정보 수정할 때 |
| 세계관 관리 | 무공, 지리, 음식, 의복 등 DB | 세계관 설정을 확인하거나 수정할 때 |
| 전략 문서 | 마스터 바이블, 집필 규칙 편집 | 핵심 설정을 변경할 때 |
| AI 교정 | 별도 빨간펜 교정 도구 | 품질 검수 외 추가 교정할 때 |

---

### 자주 묻는 질문

**Q: 수정한 내용이 다른 페이지에도 반영되나요?**
A: 네. 저장/수정하면 파일과 데이터베이스에 동시 저장되므로 모든 페이지에서 동일합니다.

**Q: 다른 컴퓨터에서 사용하려면?**
A: `https://novel-factory.onrender.com` 주소로 접속하면 됩니다. 데이터는 클라우드 DB에 있으므로 어디서든 동일합니다.

**Q: AI 검수가 오래 걸려요 (2~3분)**
A: 정상입니다. AI가 참고자료 전체를 읽고 대조하는 과정이 필요합니다.

**Q: "분석중..."이 1분 넘게 지속돼요**
A: AI 사용량 제한(Rate Limit)에 걸린 경우 자동으로 30초 대기 후 재시도합니다. 기다리면 됩니다.

**Q: 확정을 잘못 눌렀어요**
A: 현재 상태 페이지에서 해당 에피소드의 "롤백" 버튼을 누르면 되돌릴 수 있습니다.

**Q: AI 비용은 얼마인가요?**
A: 에피소드 1화 검수 + 수정 풀코스 기준 약 700원입니다.

---

## 📌 개발 로그

### 2026-02-20: v5 — 메뉴 구조 재편 + 기억카드 DB 연동 + Render 저장 수정

#### ✅ 메뉴 구조 재편 (중복 제거 + 작품/장르 분리)

**1. 사이드바 재편**
- 변경 전: 집필 흐름(4개) + 관리 도구(7개)
- 변경 후: 집필 프로젝트(5개: 현재 상태, 전략 브리핑, 작품 자료, 본문 집필, 품질 검수) + 자료실(6개)

**2. 작품 자료 (worlddb) — 이 소설 전용 22개**
- 변경 전: 세계관 DB 35개 파일 (작품 전용 + 장르 공통 혼재)
- 변경 후: 작품 자료 22개만 (지휘부, 스토리, 인물, 집필엔진, 무공, 조직, 경제, 전략방향)

**3. 장르 공통 DB (strategy) — 다른 작품에도 재사용 가능 15개**
- 변경 전: 전략 문서 10개 (마스터 바이블 등 작품 전용 혼재)
- 변경 후: 장르 공통 15개 (집필 규칙, 문체, 무공 시스템, 지리, 음식, 무기, 의복, 용어집 등)

#### ✅ 기억카드 DB 실시간 연동

**4. 현재 상태 — 기억카드 전체 표시**
- 변경 전: 최근 5개만 표시 (recent=5 하드코딩)
- 변경 후: 전체 표시, 제목 '에피소드 기억 카드 (전화)'로 변경

**5. 전략 브리핑 — 기억카드 DB 직접 연동**
- 변경 전: 마스터 파일 §7 텍스트 (12~13화에서 멈춤, 오래된 데이터)
- 변경 후: Supabase DB에서 전체 기억카드 로딩, 현재 상태와 동일한 카드 형태

#### ✅ Render 환경 저장/로딩 수정

**6. save-episode: 파일 OR DB 성공 판정**
- 변경 전: 파일 저장 실패 → success: false (Render는 파일 쓰기 불가 → 저장 실패)
- 변경 후: 파일 OR DB 중 하나만 성공해도 success: true

**7. load-episode: DB 우선 로딩**
- 변경 전: 파일 우선, DB 폴백 (Render에서 수정한 내용이 재배포 시 원상복귀)
- 변경 후: DB 우선, 파일 폴백 (네트워크에서 수정한 최신 데이터 유지)

#### ✅ UI 개선

**8. 본문 집필 설계도 화면 확대**
- 변경 전: 설계도 textarea 높이 고정 (좁음, resize 불가)
- 변경 후: min-h-500px + resize-y (넉넉한 기본 높이, 드래그로 추가 확장 가능)

**9. 사용설명서.md 생성**
- 외부 배포용 독립 사용 설명서 신규 생성
- 새 메뉴 구조 (집필 프로젝트 + 자료실) 반영

---

### 2026-02-19: v4 — AI 빨간펜 검수 통합 + 토큰 최적화

#### ✅ 수정/개선

**1. "AI 검수 실행" → 빨간펜 상세 검토로 통합**
- 변경 전: quality-check API → 6가지 점수만 표시 (수정 불가)
- 변경 후: ai-review API → 행별 이슈 카드 + "AI 수정" + "적용" 버튼
- 한 번 클릭으로 설정 오류, 말투 위반, 스토리 모순 자동 검출 + 즉시 수정

**2. 토큰 초과 해결 (208K → 200K 한도 초과 문제)**
- 참고자료 최대 8만자 제한, 이전 화 2만자 제한
- 행번호 지시 시 해당 행 ±50줄만 전송

**3. Rate Limit 자동 재시도**
- Claude 429 에러 시 30초 대기 후 자동 재시도 (최대 2회)

**4. 에러 표시 개선**
- 변경 전: API 에러 시 아무 반응 없음
- 변경 후: 에러 메시지 화면 표시 + 재시도 안내

---

### 2026-02-17: v3 — 에피소드 확정 시스템 + 전체 기억 카드 AI 주입

#### ✅ 신규 기능
- 에피소드 확정 API (확정/재확정/롤백)
- 현재 상태 페이지에 동기화 패널 추가
- 본문 집필 + 품질 검수에 확정 버튼 추가
- 본문 생성 + 전략 브리핑에 전체 기억 카드 자동 주입 (3단계 기억 구조)
- 대시보드 덮어쓰기 방지 (무의미한 값 필터링)
